...
export default {
    ...
    methods: {
        ...
        auditFalse:function(){
            let param={
                "docNo": this.newBomDetail.docNo,
                "docVersion": this.newBomDetail.docVersion
            }
            this.$http.post(__URL.dev+'product/bom/unaudit',param).then(
                (res) =>{
                    if(res.data.success){
                        this.$emit('refresh','驳回成功')
                    }else{
                        this.msgconfig.show=true;
        ...
        auditTrue:function(){
            ...
            this.$http.post(__URL.dev+'product/bom/audit',param).then(
                (res) =>{
                    if(res.data.success){
...
<template>
    ...
    <div class="inner-footer">
        ...
        <ajax-button class="btn-xl-danger" v-if='type==1 && audit'  @click='auditFalse'>驳回</ajax-button>
        <ajax-button class="btn-xl-imp" v-if='type==1 && audit' @click='auditTrue'>审核</ajax-button>
    </div>
