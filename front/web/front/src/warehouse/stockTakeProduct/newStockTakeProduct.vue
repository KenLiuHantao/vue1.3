<script>
    ...
    ready() {
        this.lock = false;
        if(this.warehouse){
            this.$http.post(__URL.warehouse + 'stock/take/product/create', {warehouseCode: this.warehouse}).then((res) => {
    ...
    methods: {
        ...
        save() {
            ...
            if(!this.lock) {
                this.lock = true;
                this.$http.post(__URL.warehouse + 'stock/take/product/save', this.tableData).then( (res) => {
                    if(res.data.success) {
                        ...
                        <tab-item>
                            <table class="inner-table">
                                ...
                                <tr v-for="(index,item) in tableData.dataList">
                                    ...
                                    <td>
                                        <input-number  :code.sync='item.firmQuantity' @change="calc($index, item.firmQuantity, null)" placeholder="请输实盘数量"></input-number>
                                    </td>
