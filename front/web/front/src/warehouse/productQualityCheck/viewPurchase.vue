<script>
    export default {
        ...
        ready() {
            this.$http.get(__URL.purchase + 'order/query/detail/' + this.docno).then((res) => {
...
<template>
    ...
    <div class="inner-footer">
        <button v-if="showbutn == 1" @click='config.show = false' class="btn-xl-def">取消</button>
        <button v-else @click='config.show = false' class="btn-xl-def">关闭</button>
        <button v-if="showbutn == 1" @click="qualityCheck" class="btn-xl-imp">品检</button>
    </div>
</template>
