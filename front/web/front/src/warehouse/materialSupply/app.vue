...
export default {
    ...
    methods: {
        ...
        /* 数据请求 */
        getList(obj) {
            var url = __URL.manufacture + 'production/material/add/list';
            switch(cfg.leftItemCfg.tabCfg.select) {
                case 'tab1':
                    url = __URL.manufacture + 'production/material/add/list';
                    break;
                case 'tab2':
                    url = __URL.manufacture + 'production/material/add/list';
                    break;
...
<template>
    ...
    <inner :config='newConfig'>
        <new-supply :selected-data.sync="selectedData" @refresh='refresh' @close="newConfig.show = false;"></new-supply>
    </inner>
