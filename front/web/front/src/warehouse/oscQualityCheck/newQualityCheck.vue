            ...
            save() {
               if(!this.lock) {
                   if (this.checkQuantity()) {
                       this.$http.post(__URL.warehouse + 'osc/quality/check/save', this.tableData).then((res) => {
                           this.warnConfig.show = true;
                        ...
                        <tab-item>
                            <table class='inner-table'>
                                ...
                                <tr v-for='item in tableData.dataList'>
                                    ...
                                    <td>{{ item.quantity }}</td>
                                    <td>
                                        <input-number class="wd100" :code.sync="item.goodQuantity | setGoodQuantity  $index"   placeholder="请输入合格允收数量"></input-number>
                                    </td>
                                    <td>
                                        <input-number  class="wd100" :code.sync="item.badQuantity | setBadQuantity  $index" placeholder="请输不良退收数量"></input-number>
                                    </td>
                                    <td><input type="text" maxlength='120' v-model='item.reason'></td>
                                    <td>
                                        <upfile-group :config='uploadFileConfig' :data="item.images" :edit="true"></upfile-group>
                                    </td>
                                </tr>
                                </tbody>
                            </table>
