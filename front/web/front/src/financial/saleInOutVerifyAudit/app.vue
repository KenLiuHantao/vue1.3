...
export default {
    ...
    methods: {
        getList(param,subParam) {
            ...
            let url = 'verify/receivable/unaudit/list';
        ...
        didAudit(type,item){
            this.sendData('verify/receivable/audit/','审核成功',item.docNo )
        },
        giveUpAudit(type,item){
            this.sendData('verify/receivable/unaudit/','弃审成功',item.docNo)
        },
...
<template>
    <container>
        ...
        <inner :config="orderCfg">
            <info :show="orderCfg" @refresh="refresh" :item-arr="actItem" :type="pageType" :add="isAdd" :edit="isEdit"></info>
        </inner>
        <!--状态消息提示-->
