    ...
    export default{
        ...
        ready() {
            ...
            if (this.invoiceCategoryCode == "002" && this.docNo.length > 0) {
                ...
                this.flowCfg.data[0].name = "创建销售退票";
                this.red = true;
                this.$http.get(__URL.financial + 'sales/invoice/red/' + this.docNo).then(
                    (res) => {
        ...
        methods: {
            ...
            showDetail(){
                //多条开票获取详情信息
                this.$http.post(__URL.financial + 'sales/invoice/wait/detail/multi', this.docNos).then(
                    (res) => {
                        this.orders = res.data.data;
                        console.log(this.orders);
                    }
            ...
            add(){
                ...
                this.$http.post(__URL.financial + 'sales/invoice/insert', this.orders).then(
                    (res) => {
                        if (res.data.success) {
                            ...
                            <tr v-for='item in orders.dataList'>
                                ...
                                <td>
                                    <input-number v-if="red" class="wd100"
                                                  ...
                                                  :style="red?'color:red':''" key="quantity"
                                ...
                                <td>
                                    <input-number v-if="!red" class="wd100" :style="red?'color:red':''"
                                                  ...
                                                  key="price" placeholder="无税单价"></input-number>
                                    <span v-else>
                                        {{item.unitPrice}}
                                    </span>
                                </td>
