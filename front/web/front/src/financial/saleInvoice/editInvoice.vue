    ...
    export default{
        ...
        methods: {
            ...
            showDetail(){
                ...
                this.$http.get(__URL.financial + 'sales/invoice/detail/' + this.docNo).then(
                    (res) => {
                        ...
                        if (this.red) {
                            this.flowCfg.data[0].name = "创建销售退票";
                            this.invoiceCfg.title = "销售退票";
                        } else {
                            this.flowCfg.data[0].name = "创建销售开票";
                            this.invoiceCfg.title = "销售开票";
                        }
            ...
            save(){
                ...
                this.saveCb = false;
                this.$http.post(__URL.financial + 'sales/invoice/update', this.orders).then(
                    (res) => {
                        if (res.data.success) {
            ...
            del(){
                ...
                this.delCb = false;
                this.$http.get(__URL.financial + 'sales/invoice/delete/' + this.docNo).then(
                    (res) => {
                        if (res.data.success) {
            ...
            submit(){
                ...
                this.$http.post(__URL.financial + 'sales/invoice/submit', this.submitVo).then(
                    (res) => {
                        if (res.data.success) {
            ...
            addAndSubmit(){
                ...
                this.$http.post(__URL.financial + 'sales/invoice/savesubmit', this.orders).then(
                    (res) => {
                        if (res.data.success) {
                            this.showMsg('success', '提交成功');
                            this.invoiceCfg.show = false;
            ...
            <div class="inner-item form-box">
                ...
                <div class="form-group">
                    ...
                    <select v-if="isEdit" v-model='orders.header.invoiceTypeCode'>
                        <option :value='item.invoiceTypeCode' v-for='item in invoiceType'>
                            {{item.invoiceTypeName}}
                        </option>
                    </select>
                    <p v-else class="orderData">{{invoiceTypeName}}
                    </p>
                </div>
                <div class="form-group">
                            ...
                            <tr v-for='item in orders.dataList'>
                                ...
                                <td :style="red?'color:red':''">
                                    <input-number v-if="red && isEdit" :style="red?'color:red':''" class="wd100"
                                            :code.sync="item.sourceQty | setQtyTax 'taxAmounts' $index" key="quantity"
                                ...
                                <td>
                                    <input-number v-if="!red && isEdit" class="wd100"
                                            :code.sync="item.unitPrice | setUnitPriceTax 'sourceQty' $index" key="price"
