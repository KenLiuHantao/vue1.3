    ...
    export default{
        ...
        methods: {
            ...
            computedData(data,context){
                data.forEach(item=>{
                    if((item.inbound != undefined && !item.inbound)|| item.cashierDocType == 3){
                        item.showCurrentAmount = item.availableAmount
                ...
                for(let i = data.length-1;i>=0;i--){
                    if(this[context] != 0){
                        if(data[i].inbound || data[i].cashierDocType != 3){
                    ...
                    res => {
                        if (res.data.success) {
                            ...
                            if (!val) {
                                ...
                                this.sendData('verify/payable/submit/', '提交审核成功', res.data.data)
                            }
                        } else {
            ...
            pushAudit(){
                ...
                if (this.edit || this.add) {
                    this.backFootData()
                    let url = __URL.financial + 'verify/payable/save/submit';
                    if (this.edit) {
                        this.allInfoData.header.verifyDocNo = this.itemArr[0].docNo
                    }
                    this.$http.post(url, this.pushData).then(
                ...
                <table class="inner-table mt-15">
                    ...
                    <template v-for="item in allInfoData.headList">
                        <tr :class="{'red':item.cashierDocType == 3}">
                            ...
                            <td v-if="add || edit">
                                <input :class="{'red':item.cashierDocType == 3}"
                                       type="text"
                                       maxlength="16"
