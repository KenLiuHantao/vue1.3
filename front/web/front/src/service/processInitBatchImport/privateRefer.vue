        ...
        methods:{
            ...
            sure() {
                this.referData.forEach(item => {
                     if (item.name == this.chooseArr) {
                         this.chooseInfo = item
                         this.chooseInfo.firstDeptName = item.name
                         this.chooseInfo.secondDeptName = ''
                     } else if (item.children.length > 0){
                         item.children.forEach(it => {
                             if(it.name == this.chooseArr) {
                                 this.chooseInfo = it
                                 this.chooseInfo.firstDeptName = item.name
                                 this.chooseInfo.secondDeptName = it.name
                             }
                         })
                     }
                })
                ...
                <ul class="dep-list">
                    <li v-for="item in referData" :class="{'active':currentIndex == $index}" @click="checkThisLink($index)">
                        <div class="li-content">
                            ...
                            <!--<input @click.stop="" type="radio" :value="item.name" v-model="chooseArr">-->
                        </div>
                        <ul v-if="item.children.length > 0"  @click.stop="">
                            <li v-for="sonItem in item.children">
                                <div class="li-inner">
                                    <span>{{ sonItem.name }}</span>
                                    <input @click.stop="" type="radio" :value="sonItem.name" v-model="chooseArr">
                                </div>
                            </li>
                        </ul>
                    </li>
