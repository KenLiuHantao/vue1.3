    ...
    methods: {
            ...
            if (this.activeData) {
                if (this.activeData.parentCode === '') {
                    ...
                    for (var i = 0; i < this.initData.length; i++) {
                        if (this.initData[i].code == parentCode) {
        ...
        getChildNode: function() {
            if (this.activeData) {
                var childNode = [];
                for (var i = 0; i < this.initData.length; i++) {
                    if (this.initData[i].parentCode === this.activeData.code) {
            ...
            if(keyword!=''){
                ...
                if(result.length==0){
                    ...
                    function findfather(data,that){
                        if(data.parentCode!==''){
                            for(var i=0;i<that.initData.length;i++){
                                if(that.initData[i].code===data.parentCode){
                    ...
                    function findchild(data,that){
                        if(data.leaf!=true){
                            for(var j=0;j<that.initData.length;j++){
                                if(that.initData[j].parentCode===data.code){
                    ...
                    for(var i=0;i<result.length;i++){
                        if(!test[result[i]['code']]){
                            lastResult.push(result[i])
                            test[result[i]['code']]=1;
                        }
                    }
    ...
    events:{
        ...
        checkboxdata:function(data,type){
            if(type){
                ...
                for(var i=0;i<this.checkboxData.length;i++){
                    if(this.checkboxData[i].code==data.code){
        ...
        insertdata:function(){
            if(this.config.rightcheckbox){
                var codeArray=[];
                for(var i=0;i<this.checkboxData.length;i++){
                    codeArray.push(this.checkboxData[i].code)
                }
