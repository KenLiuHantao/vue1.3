    ...
    export default {
        ...
        methods: {
            sure(){
                ...
                this.$http.post(__URL.arc + 'assetstype/update', this.tableData).then(
                    (res) => {
                        if (res.data.success) {
                            this.close();
        ...
        watch:{
            code:{
                handler(val,oldVal){
                    ...
                    //查询记录code
                    this.$http.get(__URL.arc + 'assetstype/find/'+this.code).then(
                        res => {
                            if (res.data.success) {
...
<template>
    <modal :config="detail" @sure='sure' @close='close' @quite="close">
        ...
        <div class="form-group wd-100">
            <label>资产类别:</label>
            <span>{{tableData.assetsTypeName}}</span>
        </div>
