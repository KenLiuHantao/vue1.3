<script>
    ...
    export default {
        ...
        return {
            ...
            freezeCfg:{
                ...
                title:'冻结供应商提示',
                msg:'',
                subMsg:`冻结该供应商后，将不能进行该供应商的报价、采购下单作业，是否立即冻结？`
            },
            //供应商分类弹框显示和隐藏
            supplierShow:{
                show: false,
    ...
    props:['detail'],
        ...
        //冻结
        ensureFreeze(){
            this.supplierUpdateDate.supplierStatus = 2;
            this.freezeCfg.show =true;
        },
        freezeSupplier(){
            this.$http.post(__URL.arc + 'supplier/updateSupplierStatus',this.supplierUpdateDate).then(
                    (res) => {
                if(res.data.success){
                    this.freezeCfg.show=false;
//                    this.refresh('')
                    this.$emit("refresh","冻结成功")
                }else{
...
<template>
    ...
    <div class="inner-footer">
        ...
        <button  @click='ensureFreeze' class="btn-xl-imp">冻结</button>
    </div>
    <refer-employee :config='codeModal' @cb='pickPerson' ></refer-employee>
    <alert :config='freezeCfg' @cb='freezeSupplier' ></alert>
    <!-- </modal> -->
    <!-- 业务员弹框 end -->
    <!-- 供应商分类弹框 start -->
