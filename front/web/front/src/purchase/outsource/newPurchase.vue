<script>
    ...
    ready() {
        ...
        this.$http.post(__URL.purchase + 'outsource/await/query/selected/details', params).then((res) => {
    ...
    methods: {
        ...
        save() {
            if(!this.lock) {
                this.lock = true;
                this.$http.post(__URL.purchase + 'outsource/save', this.tableData).then((res) => {
                    if(!res.data.success) {
        ...
        saveOrder() {
            if(!this.lock) {
                this.lock = true;
                this.$http.post(__URL.purchase + 'outsource/add', this.tableData).then((res) => {
                    if(!res.data.success) {
                        this.showMsg('failure', res.data.errMsg);
                        this.lock = false;
                    } else {
                        ...
                        <td>
                            <div class="show-wrap">
                                ...
                                <div class="show-input-group">
                                    <div class="title"><span class='must-point'>*</span>单价: </div>
                                    <input-group :config='priceCfg' :data.sync="item.taxUnitPrice" @modal="doSelectPrice(item)"
