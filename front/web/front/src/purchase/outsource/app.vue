<script>
    ...
    methods: {
        ...
        /* 数据请求 */
        getList(obj) {
            var url = __URL.purchase + 'outsource/await/query/list';
            switch(cfg.leftItemCfg.tabCfg.select) {
                case 'tab1':
                    url = __URL.purchase + 'outsource/await/query/list';
                    ...
                    break;
                case 'tab2':
                    url = __URL.purchase + 'outsource/query/list';
                    break;
                case 'tab3':
                    url = __URL.purchase + 'outsource/query/list';
                    break;
        ...
        inventorySure(val){
            for(var index in this.leftItemCfg.filterCfg){
                ...
                if(this.leftItemCfg.filterCfg[index].key == 'inventorySpec'){
                    this.leftItemCfg.filterCfg[index].value = val.inventorySpec;
                }
            }
        },
        openRef(emit) {
        ...
        saveOrders() {
            ...
            if(!this.lock && params.length) {
                this.lock = true;
                this.$http.post(__URL.purchase + 'outsource/selected/save', params).then((res) => {
                    if(!res.data.success) {
