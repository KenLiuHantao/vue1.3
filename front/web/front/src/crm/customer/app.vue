        ...
        methods: {
            ...
            href(item, type) {
                if (type == 'customerName') {
                    if (this.tab == 'tab1') {
                        this.$http.get(__URL.osiu + 'customer/detail/' + item.customerCode).then(
                            (res) => {
                                this.customerdetail = res.data.data;
                                this.detailConfig.show = true;
            ...
            getList() {
                if (this.tab == 'tab1') {
                    this.$http.post(__URL.osiu + 'customer/list', this.tableParams).then(
                        (res) => {
...
<template>
    ...
    <inner :config='detailConfig'>
        <customer-detail :detail='customerdetail' @refreshTable="refresh" @refresh='closeCustomerDetail'
                         :disabled='disabled' :edit='edit'></customer-detail>
    </inner>
