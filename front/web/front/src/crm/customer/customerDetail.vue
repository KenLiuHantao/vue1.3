    ...
    export default {
        data() {
            return {
                ...
                cdCfg: {
                    show: false,
                    msg: this.detail.reference == 1 ?'该客户还有未完成的订单，是否确认要停用？':'确认停用此客户档案？',
                    subMsg: '',
                    type: 'failure'
                },
                aCfg:{
        ...
        methods: {
            ...
            saveCustomer(ind){
                ...
                if (this.tab == 'tab1') {
                    if (ind == 0) {
                        this.saveAjax(__URL.osiu + 'customer/update', data);
                    } else if (ind == 1) {
                        this.newCustomer.code = this.newCustomer.id;
                        this.saveAjax(__URL.crm + 'customer/add', data, ind);
                    }
                } else if (this.tab == 'tab2') {
                    if (ind == 1) {
                        this.saveAjax(__URL.crm + 'customer/update', data);
                    }
                }
            },
            saveAjax(url,param,ind){
                this.ajaxlock = false;
                this.$http.post(url, param).then(
                    (res) => {
                        ...
                        if (res.data.success) {
                            ...
                            this.edit = false;
                            if(this.tab == 'tab1' && ind == 1){
                                this.wCfg.show = false;
                                this.$emit('refresh')
                            }
                        } else {
                            this.msgconfig.show = true
            ...
            //进入编辑状态
            enterEdit(){
                if(this.newCustomer.customerStatusCode != 'BU0230001' && this.newCustomer.reference == 1){
            ...
            refreshDetail(){
                if(this.tab == 'tab1'){
                    this.$http.get(__URL.osiu + 'customer/detail/' + this.newCustomer.customerCode).then(
                        (res) => {
            ...
            //删除当前客户档案
            deleteCustomer(){
                this.dlCfg.show = true;
            },
            sureDeleteCustomer(){
                this.$http.get(__URL.osiu + 'customer/delete/' + this.newCustomer.customerCode).then(
                    (res) => {
                        this.dlCfg.show = false;
                        if (res.data.success) {
...
<template>
    <div class="inner-wrapper">
        ...
        <alert @cb='sureDeleteCustomer' :config="dlCfg"></alert>
        <alert @cb='sureDisableCustomer' :config="cdCfg"></alert>
        <!-- 编辑状态 -->
    ...
    <div class="inner-footer">
        ...
        <button v-else @click="cancel" class="btn-xl-def">取消</button>
        <button v-if='edit==true && tab == "tab1"' @click='saveCustomer(0)' class="btn-xl-imp">保存草稿</button>
        <button v-if='edit==true && tab == "tab1"' @click='saveCustomer(1)' class="btn-xl-imp">提交</button>
        <button v-if='edit==true && tab == "tab2"' @click='saveCustomer(1)' class="btn-xl-imp">保存</button>
        <button v-if='edit==false && disabled==false && tab == "tab2"' @click='disableCustomer' class="btn-xl-danger">停用</button>
        <button v-if='edit==false && disabled==false && tab == "tab1"' @click='deleteCustomer' class="btn-xl-danger">删除</button>
