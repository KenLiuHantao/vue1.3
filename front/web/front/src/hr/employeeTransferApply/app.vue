<script>
    ...
    export default {
        ...
        methods: {
            ...
            closeTransfer(){
                this.choiceTab(0)
                this.transferConfig.show=false
                this.transferDetailConfig.show=false
            },
            //关闭兼岗内页
            closePart(){
                this.choiceTab(0)
                this.partConfig.show=false
            ...
            // 新增调岗和兼岗的方法
            changeRole(){
                this.typeModalCfg.show = false
                this.transferConfig.show = true
            },
            partRole () {
                this.typeModalCfg.show = false
                this.partConfig.show = true
            },
            // 列表选项卡切换
            changeTab (id) {
                if(id === 'tab1'){
...
<template>
        ...
        <div class="body_item">
            <div class="left_item">
                <div class="item_head between">
                    <a @click='typeModalCfg.show = true'><i class="icon-hp_list__add"></i>新增</a>
                    <a @click='filterMod' href="javascript:;"><!--<i class="icon-filter-two"></i>筛选--></a>
                </div>
    ...
    <!--筛选弹框 -->
    <modal :config.sync='typeModalCfg'>
        <div class="modal-input-wrap">
            <div @click="changeRole" class='item'>
                <img src="../../../../../public/images/employeeTransferApply_type1.png" alt="调岗">
                <div>调岗</div>
            </div>
            <div @click="partRole" class='item'>
                <img src="../../../../../public/images/employeeTransferApply_type2.png" alt="兼岗">
                <div>兼岗</div>
            </div>
        </div>
    </modal>
    ...
    <!--调岗申请新增内页-->
    <inner :config='transferConfig'>
        <new-transfer :show.sync='transferConfig.show' :msg-cfg="msgCfg" @refresh='closeTransfer'></new-transfer>
    </inner>
    <!--兼岗申请新增内页-->
    <inner :config='partConfig'>
        <new-part :show.sync='partConfig.show' :msg-cfg="msgCfg" @refresh='closePart'></new-part>
    </inner>
