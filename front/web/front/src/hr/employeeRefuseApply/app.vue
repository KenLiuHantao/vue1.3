<script>
    ...
    import newRefuse from './newRefuse'
    export default {
        components: {
            checkEmployee,
            detail,
        ...
        methods: {
                ...
                this.$http.post(__URL.hr + 'employee/refuse/application/list', param).then(
                    (res) => {
                        if(res.data.success){
                            ...
                            this.totalCount=res.data.data.totalCount;
                            this.consoleStatus = param.rejectStatus
                        }else{
                            this.msgconfig.show=true
                            this.msgconfig.content=res.data.errMsg
                        }
            ...
            //获取tab数量
            getTabNumber(){
                this.$http.post(__URL.hr+ 'employee/refuse/application/findTabCount',{roleCode:0}).then(
                    (res)=>{
                        if(res.data.success){
                            for(var i=0;i<res.data.data.dataList.length;i++){
                                if(res.data.data.dataList[i].tableIndex===2){
                                    ...
                                    this.tabCount.count0=res.data.data.dataList[i].count;
                                }
                            }
                        }else{
...
<template>
    ...
    <!--辞退申请新增内页-->
    <inner :config='refuseConfig'>
        <new-refuse :show.sync='refuseConfig.show' @refresh='closeRufuseEmployee' :msg-cfg="msgconfig"></new-refuse>
    </inner>
    <inner :config='detailConfig'>
        <detail :show.sync='detailConfig.show' :msg-cfg="msgconfig" :detail-list.sync='detailList' @refresh='closeDetailEmployee'></detail>
    </inner>
