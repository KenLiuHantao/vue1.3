...
export default {
    ...
    methods: {
        ...
        //获取tab数量
        getTabNumber(){
            this.$http.post(__URL.hr+ 'employee/dimission/application/findTabCount/'+ 1,{}).then(
                (res)=>{
                    if(res.data.success){
        ...
        getDetail(type,item){
            this.$http.get(__URL.hr + 'employee/dimission/application/detail/' + item.recordId).then(
                (res) => {
                    if(res.data.success){
