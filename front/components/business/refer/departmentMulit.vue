            ...
            checkAllChild (item) {
                ...
                if (index === -1) {
                    ...
                    item.children.forEach(obj => {
                        let sonIndex = this.chooseArr.indexOf(obj.code)
                        if (sonIndex === -1) {
                            this.chooseArr.push(obj.code)
                        }
                    })
                } else {
                    ...
                    item.children.forEach(obj => {
                        let sonIndex = this.chooseArr.indexOf(obj.code)
                        if (sonIndex !== -1) {
                            this.chooseArr.splice(sonIndex, 1)
                        }
                    })
                }
            },
                ...
                <ul class="dep-list">
                    <li v-for="item in listData" :class="{'active':currentIndex == $index}" @click="checkThisLink($index)">
                        <div class="li-content">
                            ...
                            <input @click.stop="checkAllChild(item)" :value="item.code" v-model="chooseArr" type="checkbox">
                        </div>
                        <ul v-if="item.children.length >0"  @click.stop="">
                            <li v-for="sonItem in item.children">
                                <div class="li-inner">
                                    <span>{{sonItem.name}}</span>
                                    <input @click.stop="isCheckAll(sonItem, item)" :value="sonItem.code" v-model="chooseArr" type="checkbox">
                                </div>
                            </li>
                        </ul>
                    </li>
