<script>
    import tree from 'nc/tree/vtree'
    export default{
        props:['config'],
        data(){
            return{
        ...
        methods:{
            setTree:function(){
                for(var i=0;i<this.topTreeData.length;i++){
                    this.topTreeData[i].code=this.topTreeData[i].customerCategoryCode;
                    this.topTreeData[i].name=this.topTreeData[i].customerCategoryName;
                    this.topTreeData[i].parentCode=''
                }
            },
            getInitData:function(data){
                this.$http.get(__URL.crm + 'customer/industry/ref').then(
                    (res) => {
            ...
            delThisBill:function(){
                ...
                this.ajaxlock=false;
                this.$http.get(__URL.arc + 'customer/category/delete/'+this.deldata.code).then(
                    (res) => {
                        this.ajaxlock=true;
                        if(!res.data.data){
        ...
        events:{
            ...
            savedata:function(data,name){
                ...
                if(data.name==''){
                    ...
                    this.ajaxlock=false;
                    let params = {
                        "customerCategoryName":name,
                        "remark":""
                    }
                    this.$http.post(__URL.arc + 'customer/category/insert',params).then(
                        (res) => {
                            this.ajaxlock=true;
                            if(!res.data.data){
                                this.msgconfig.content = res.data.errMsg
                    ...
                    var paramData={
                        "customerCategoryCode": data.code,
                        "customerCategoryName": name
                    }
                    ...
                    this.ajaxlock=false;
                    this.$http.post(__URL.arc + 'customer/category/update', paramData).then(
                        (res) => {
                            this.ajaxlock=true;
                            if(!res.data.data){
                                this.msgconfig.content = res.data.errMsg
...
<template>
    <modal :config="modalCfg">
        <div class="tree-wrap">
            <div class="tree-head" :class="{'border-none':!treeShow}">
                <div>
                    <span :class="{'icon-contract':treeShow,'icon-deploy':!treeShow}" @click="toggleTree"></span>客户分类
                </div>
                <div>
            ...
            <div class='blank' v-show='topTreeData.length==0 && !loadingShow'>
                ...
                <!--<p>无客户分类内容，请<a href='javascript:;' @click='addChild'>新增</a></p>-->
                <p>无客户分类内容</p>
            </div>
        </div>
        <message :config="msgconfig" ></message>
        <alert :config="delConfig" @cb="delThisBill">您将删除当前客户分类，删除后该数据将无法再找回。</alert>
    </modal>
</template>
