...
let treeBox = Vue.extend({
    ...
    template: "<ul class='table-ul'>" +
        ...
        "            </div>" +
        "            <div class='wd1'>" +
        "                <span v-if='edit'>订单数量:{{proVo.manuQuantity}}</span>" +
        "                <p v-if='!edit'>订单数量:{{proVo.manuQuantity}}</p>" +
        "            </div>" +
        "            <div class='wd1 double'>" +
        ...
        "    </li>" +
        "    <li class='bodyHead'>" +
        "        <div class='ser'>序号</div>" +
        "        <div class='wd1 bdhead' v-for='wd in words' track-by='$index'>{{wd}}</div>" +
        "    </li>" +
        "    <list v-for='item in bomData' :ind='$index' :data.sync='item' :manu='proVo.manuQuantity' :edit='edit' @del='del($index)' :fromsale='fromsale'></list>" +
        "</ul>" +
    ...
    computed: {
        ...
        words(){
            if(this.fromsale){
                return ['货品信息', '单位', '单位用量', '订单用量', '损耗率（%）', '生产计划用量', '', '']
            }else{
                return ['货品信息', '单位', '单位用量', '订单用量', '损耗率（%）', '生产计划用量', '供应商信息', '']
            }
        }
    },
    filters: {
