...
import pro from 'nb/refer/supplier.vue'
import cfg from './cfg.js'
import alert from 'nc/modal/alert.vue'
import message from 'nc/modal/msg.vue'
export default {
    components: {
        inp,
        inv,
    ...
    props: ['data', 'ind', 'edit', 'manu','fromsale'],
    data() {
        return {
            openLast: false,
            children: {},
    ...
    ready(){
        this.beforeChangeAttributeCode=this.data.attributeCode;
    },
    methods: {
        isType(type) {
            return function(obj) {
        ...
        multi(a, b) {
            ...
            let adice = a.toString().split('.').length > 1 ? a.toString().split('.')[1].length : 0,
                ...
                row = parseInt(newa * newb) / Math.pow(10, adice + bdice)
        ...
        getInv(item) {
            ...
            if("1" == item.attributeCode || "2" == item.attributeCode){
                this.$http.get(__URL.dev + 'order/bom/get/exist/1/'+item.inventoryId).then(
                    (res) => {
        ...
        changeAttribute(){
            this.changeCfg.msg='是否将该货品的属性更改为'+this.beforeChangeAttributeName+'?'
            this.changeCfg.show=true
        },
        sureChange(){
            this.data.attributeCode=this.beforeChangeAttributeCode;
            this.data.attributeName=this.beforeChangeAttributeName;
            //根据变更的不同属性对数据进行处理 0采购件 没有子件 1自制件 2委外件 子件没有工序
            //不用了。。 只要变化了属性就要清空下面的数据
            this.data.last=false;
            this.openLast = false;
            this.data.childBom=null;
        },
        cancelChange(){
            this.beforeChangeAttributeCode=this.data.attributeCode;
            this.beforeChangeAttributeName=this.data.attributeName;
        }
    },
    filters: {
        quantity(val) {
    ...
    computed: {
        ...
        beforeChangeAttributeName(){
            if(this.beforeChangeAttributeCode==0){
                return '采购件'
            }else if(this.beforeChangeAttributeCode==1){
                return '自制件'
            }else if(this.beforeChangeAttributeCode==2){
                return '委外加工件'
            }
        }
    }
    ...
    <li class='bodyCnt'>
        ...
        <div class='wd1'>
            ...
            <div class='itemGroup'>
                ...
                <select v-if='edit' @change='changeAttribute' v-model='beforeChangeAttributeCode'>
        ...
        <div class='wd1' v-show='fromsale || data.attributeCode==1'></div>
        <div class='wd1' v-show='!fromsale && data.attributeCode!=1'>
            <div v-if='edit' class='s-input'>
        ...
        <div class='wd1 cnt'>
            <button v-if='edit && !hasChild && (data.attributeCode ==1)' @click='addProBox' class="button-primary">新增工序</button>
            <button v-if='edit && !hasChild && (data.attributeCode ==2)' @click='addProBox' class="button-primary">新增子件</button>
            <button v-if='!hasChild && edit' @click='delItem' class="button-danger">删除</button>
        </div>
    </li>
    <template v-if='hasChild && openLast'>
        <tret :data.sync='data.childBom' :edit='edit' :fromsale='fromsale'></tret>
    </template>
