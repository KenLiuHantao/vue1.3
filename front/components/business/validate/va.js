...
// 自定义规则
function personalVali(init, obj) {
    let dom = domType(init.el),
        name = dom.name,
        conditions = init.vm.vaConfig[name]

    if (init.vm.vaReady && obj == '') {
        init.vm.vaReady = false
    } else {
        ...
        if (isArray(obj)) {
            let isInit = init.vm.vaConfig[name].length
            ...
            if (isInit > 1) valiOwn(init, obj[0])
        }
...
function valiOwn(obj, val) {
    let vm = obj.vm, //上下文 ViewModel
        name = obj.arg == undefined ? 'nonvoid' : obj.arg, //参数
        el = domType(obj.el), //dom节点
        tag = el.getAttribute('tag'), //tag属性名称
        option = obj.modifiers //修饰符
    ...
    if (_result) {
        ...
        typeof _result === 'string' ? alert(_result) : domErr(el, conditions[0].tag, _result)
...
va.install = (Vue) => {
    Vue.directive('va', {
        bind() {
            let _ = this,
                vm = this.vm, //上下文 ViewModel
                name = this.arg == undefined ? 'nonvoid' : this.arg, //参数
                tag = this.el.getAttribute('tag'), //tag属性名称
                el = domType(this.el), //dom节点
                baseCfg = [], //默认的校验规则    --不用写，默认存在的规则（如非空）
                optionalConfig = [], //用户选择的配置成套    --与name相关
                customConfig = [], //用户自定义的规则（组件中）  --bingding.value
                option = this.modifiers, //修饰符
                errMsg = ''

            vm.vaReady = true
            el.classList.add('va' + vm._uid)
            el.name = name
            ...
            //用户自定义的规则
            if (this.value) {
                customConfig = this.value.map(item => {
